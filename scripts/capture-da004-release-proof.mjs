...
const hero = "https://res.cloudinary.com/szvtq9d8/image/upload/v1789455274/dead-air/da-004/publication/da004_art001_v2_3_16x9_1600x900.webp";
const mobile = "https://res.cloudinary.com/szvtq9d8/image/upload/v1789455282/dead-air/da-004/publication/da004_art001_v2_3_2x3_1024x1536.webp";
...
const allAssets = [hero,mobile,card,square,og,social];
...
const verify = async (page,label,isMobile) => {
  ...
  if(isMobile){assert.equal(current.src,mobile);assert.equal(current.nw,1024);assert.equal(current.nh,1536);} else {assert.equal(current.src,hero);assert.equal(current.nw,1600);assert.equal(current.nh,900);}
  const preload=page.locator(`link[rel="preload"][as="image"][href="${isMobile?mobile:hero}"]`);
  assert.equal(await preload.count(),1,`${label} responsive preload missing`);
  const resources=await page.evaluate((src)=>performance.getEntriesByName(src).length,current.src);
  assert(resources>0,`${label} hero resource timing missing`);
  ...
  for(let i=0;i<await h2.count();i++) if(/^\d+\.\s/.test((await h2.nth(i).innerText()).trim())) numbered++;
...
const verifyPublicationSurfaces = async (page) => {
  ...
  for(const url of allAssets){const r=await page.request.get(url);assert(r.ok(),`Missing approved WebP ${url}`);assert.match(r.headers()["content-type"]??"",/image\/webp/);}
};
...
try {
  ...
  console.log("DA-004 release proof PASS: public indexability, publication date, canonical/OG/Twitter metadata, physical-plaque responsive WebP art, exact alt, desktop/mobile crop, semantics, eager/high-priority hero, resource timing, archive/feed surfaces, and ten story sections verified.");
