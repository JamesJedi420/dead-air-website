---
slug: da-002-the-name-in-the-room
title: The Name in the Room
summary: A documentary crew returns to Bellweather High for a final session with a medium. In the basement, equipment fails, a fire door moves, and a recorded sound leaves much to interpretation.
status: withheld
classification: Literary paranormal horror
readingTime: 88–111 minutes
canonicalStatus: established canon
draft: true
tags:
  - literary paranormal horror
  - documentary horror
  - institutional horror
  - psychological horror
  - haunted school
phenomenon:
  - disputed mediumship
  - contested audio
  - disputed visual evidence
  - equipment failure
  - fire-door movement
  - ritual relief
  - collaborative identity formation
evidenceType:
  - continuous video
  - interrupted video
  - facilities audio
  - production audio
  - device logs
  - handling records
  - position maps
  - environmental readings
  - interviews
  - source comparisons
  - chain of custody
locations:
  - Bellweather High School
contentWarnings:
  - Group psychological pressure
  - Grief and private memory
  - Disputed mediumship and paranormal claims
  - Ambiguous audio and images
  - Claustrophobic basement and crawlspace spaces
  - References to bullying, school humiliation, a car crash, and death
contentNotes:
  - No graphic violence.
cases:
  - da-002
characters:
  - diane-madsen
  - evan-kruse
  - abby-larson
  - ron-meier
  - miriam-vale
objects:
  - unidentified-wet-brass-key
mysteries:
  - cause-of-the-da-002-fire-door-movement
  - source-and-content-of-the-da-002-audio
  - scope-and-effect-of-the-bellweather-return-ritual
  - identity-and-source-of-the-auditorium-impression
  - relationship-among-the-recorded-phenomena-at-bellweather-high
---

<!--
PRIVATE PREVIEW SOURCE GATE

The complete Final Approved Story v12 remains in the controlled Google Drive website-edition source during this fail-closed repository stage. This withheld entry intentionally contains no manuscript body. Before publication, the approved body must be imported, compared against the Drive source, rendered in a private deploy preview, proofed in desktop and mobile layouts, and approved separately. Changing draft or status without completing that sequence is prohibited.
-->
